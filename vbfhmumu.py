...
for sampleName in samples:
    for datasetNumber,dataset in enumerate(samples[sampleName]):
	    ...
	    inputFile = open("%s/%s.%s.%s.input.txt"%(prod,sampleName,datasetNumber,fileGroupNumber),'w')
	    for f in fileGroup:
                inputFile.write("%s \n"%f)
            inputFile.close()
	    hostname_job = htcondor.Submit({
            "executable": "skim.sh",  # the program to run on the execute node
            "arguments": "%s_%s_%s.root, %s.%s.%s.input.txt"%(sampleName,datasetNumber,fileGroupNumber,sampleName,datasetNumber,fileGroupNumber),  # anything the job prints to standard output will end up in this file
	    ...
	    "transfer_input_files": "converter.py, variables.py, skim.sh, %s.%s.%s.input.txt"%(sampleName,datasetNumber,fileGroupNumber),
	    #"+JobFlavour": "nextweek",
	    "+MaxRuntime" : "120000",           
